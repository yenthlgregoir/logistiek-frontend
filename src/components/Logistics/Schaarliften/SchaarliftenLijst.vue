...
.search input { width: 100%; padding: 8px 12px 8px 36px; border-radius: 8px; border: 1px solid #d1d5db; }
.search i { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: #6b7280; }
.clickable { cursor: pointer; font-weight: 500; }
.expanded-row-container { display: flex; gap: 20px; padding: 20px; border-radius: 8px; }
.expanded-row-left, .expanded-row-right { flex: 1; display: flex; flex-direction: column; }
...
.keuring > div:nth-child(1) { font-weight: 700; }
progress { grid-column: 1 / -1; width: 80%; height: 10px; border-radius: 5px; overflow: hidden; appearance: none; }
progress::-webkit-progress-bar { background-color: #ddd; border-radius: 5px; }
progress::-webkit-progress-value { background-color: #3b82f6; border-radius: 5px; }
.expired-text { color: #b91c1c; font-weight: 700; grid-column: 1 / -1; padding: 6px 0; font-size: 14px; }
.coming-soon-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; font-size: 18px; font-style: italic; color: #888; border: 2px dashed #ccc; border-radius: 8px; user-select: none; min-height: 150px; }
...
.devider { width: 100%; border: 1px solid rgb(194, 190, 190); margin-bottom: 1rem; }
